...
mod tests {
    ...
    // Pure tool-call-recovery-logic tests (tool_call_from_content,
    // maybe_tool_call_json, etc.) live in
    // `src/llm/provider/tool_call_recovery.rs` since
    // llama-cpp-2-integration-plan.md Phase 4 - this module only keeps the
    // end-to-end tests below, which exercise OllamaProvider's own wiring
    // (from_ollama_response) around that shared logic, not the logic itself.
