...
fn tool_call_from_content(content: &str, offered: &[Tool]) -> Option<(String, serde_json::Value)> {
    ...
    // Case 2: the call is a ```json-fenced block embedded in prose. Some models
    // (qwen2.5-coder after a rejected call) explain themselves and wrap the
    // retry call in a fence: "Let's try again:\n```json\n{...}\n```". Recover
    // the FIRST fenced block that parses as a call to an offered tool; the loop
    // re-prompts for any further calls. Only *fenced* blocks are considered -
    // a bare `{...}` mentioned inline in prose is deliberately NOT treated as a
    // call (that would fire on a model merely describing JSON).
...
/// block in `content`, in order. Used to find tool calls a model wrapped in a
/// code fence inside otherwise-prose content.
...
/// Parse a single JSON object as a strict tool call: it must name an offered
/// tool, carry an explicit `arguments`/`parameters` object, and have no other
/// keys. Shared by both the whole-message and embedded-fence recovery paths.
...
mod tests {
    ...
    /// End to end for the reported bug: qwen2.5-coder apologised in prose and
    /// wrapped the retry `bash` call in a ```json fence. It must become a
    /// ToolUse block (executed), not be printed to the user as raw JSON.
    #[test]
    fn fenced_call_in_prose_becomes_a_tool_use_block() {
        let provider = OllamaProvider::default_local();
        let content = "I apologize for that. Let's try creating the folder \
             without using shell operators.\n\nFirst, let's create the folder:\n\n\
             ```json\n{\"name\": \"bash\", \"arguments\": {\"command\": \"mkdir exercice1\"}}\n```";
        let response = mock_response(ChatMessage::assistant(content.to_string()), true);
        ...
        let tool_uses: Vec<_> = llm
            ...
            .filter(|b| matches!(b, ContentBlock::ToolUse { .. }))
            .collect();
        assert_eq!(tool_uses.len(), 1, "the fenced call must be executed");
        assert!(
            !llm.content
                .iter()
                .any(|b| matches!(b, ContentBlock::Text { .. })),
            "the recovered call must not also be shown as prose"
        );
    }
