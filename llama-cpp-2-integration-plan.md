...
Status: **All phases implemented, 0–10 including 4b** (streaming MVP with
sampling/context fixes, tool calling (now grammar-constrained via a
mid-stream sampler swap, `llama-cpp-llguidance` feature), model management,
TUI integration, GPU backend features, idle-unload, and docs/ADR; §0.1's
Go/No-Go gate was explicitly overridden by the repo owner to proceed — see
below). Phase 4b's mid-stream swap was implemented after the repo owner
explicitly asked to proceed despite the unverified-without-a-real-model risk
flagged when the infrastructure alone first landed — see that phase's row
for the full reasoning on why it's safe to ship unverified (the failure
mode is a silent, non-crashing degrade to pre-existing behavior, not a hang
or corruption).
...
| Phase 2 — Streaming | ✅ Done | `Provider::stream()` implemented: `InferenceJob` became an enum (`Complete`/`Stream`); the `Stream` variant carries an `mpsc::UnboundedSender<Result<StreamEvent>>` that the worker fills token-by-token as it generates, wrapped as a `tokio_stream::wrappers::UnboundedReceiverStream` — genuinely incremental (`stream()` returns before generation starts), verified against `AgentService::drain_stream_to_response`'s per-item `.await` consumption. Event sequence matches every other provider (`MessageStart → ContentBlockStart → ContentBlockDelta* → ContentBlockStop → MessageDelta → MessageStop`). Added `drain_valid_utf8()` to handle multi-byte characters split across token boundaries (5 offline tests, including CJK reassembly). Cancellation handled: a dropped consumer makes `events_tx.send()` fail, and `run_stream` returns immediately instead of continuing to generate unwatched tokens — the worker goes back to `blocking_recv()` for the next job, matching the Phase 2 exit criterion. `supports_streaming()` now `true`. 8/8 tests pass under `--features llama-cpp`; 783/783 unaffected on the no-feature and `all-llm` builds. |
| Phase 3 — Sampling, context, chat templates | ✅ Done | Audited against Phase 1: temperature/top_p/top_k/repeat_penalty/seed wiring, chat-template resolution, stop-sequence matching, and `max_tokens` enforcement were already delivered in Phase 1 — this phase closed the three real gaps that audit found. **(1)** `context_window()` returned the *configured* `n_ctx`, not the context's actually-resolved one (a user setting `n_ctx = 0`, llama.cpp's "use the model's trained default" signal, would silently diverge from what was really allocated) — fixed by capturing `context.n_ctx()` right after context creation and propagating it back through the ready channel, satisfying the exit criterion's exact invariant. **(2)** `request.frequency_penalty`/`presence_penalty` were silently dropped (penalties sampler hardcoded to `0.0`/`0.0`) — now wired through. **(3)** `request.seed` (u64) was truncated with `as u32`, which could silently reproduce a different sequence than asked for — changed to a fallible `u32::try_from`, matching `OllamaProvider`'s own handling of the same generic field (an oversized seed is dropped, not corrupted). All FFI-level fixes with no new pure logic to offline-unit-test; correctness needs the manual test plan below. |
| Phase 4 — Tool calling | ✅ Done | Extracted `OllamaProvider`'s private recovery functions (`maybe_tool_call_json`, `tool_call_from_content`, `fenced_json_blocks`, `parse_tool_call_object`) into the new, always-compiled `src/llm/provider/tool_call_recovery.rs` — `OllamaProvider` now calls the shared module (behavior-preserving; its 7 pure tests moved with it and pass unmodified, its 2 end-to-end tests stayed and still pass). `LlamaCppProvider::supports_tools()` is now `true`: `build_prompt()` injects offered tools as a JSON-function instruction block into the system prompt (new pure `merged_system_prompt`/`tool_instructions_block` helpers); `run_complete()` attempts recovery on the full generated text; `run_stream()` replicates `OllamaProvider::stream()`'s withholding pattern exactly — text that still looks like printed JSON is held back from `ContentBlockDelta` until it's disambiguated, and the text `ContentBlockStart` itself is now lazy so an all-tool-call response never shows an empty text block. 14/14 `llama_cpp` tests, 7/7 `tool_call_recovery` tests, 37/37 `ollama` tests (unmodified). No-feature build: 790/790 (the shared module is always compiled). `all-llm` unaffected. |
| Phase 5 — Performance metrics | ✅ Done (as of Phase 1/2) | `PerfMetrics` populated on both `complete()` and `stream()` from `context.timings()` (load/prefill/eval durations), flowing through the existing provider-agnostic `LLMResponse`/`StreamEvent::MessageDelta` pipeline unchanged — no separate implementation work was needed once Phases 1-2 landed, matching the plan's own note that this phase was expected to be quick. Not yet exercised against a real model or the TUI (see the manual-test gap below). |
| Phase 6 — Model management | ✅ Done | `src/llm/provider/llama_cpp_models.rs`: `LocalGgufModel`/`DownloadProgress` (named to match `ollama_models.rs`'s `LocalModelInfo`/`PullProgress` field-for-field), `list_local_models()` (directory scan, best-effort quantization-tag guess from the filename), `resolve_download_source()` (`hf:org/repo/file.gguf` shorthand → HF `resolve/main` URL + a best-effort SHA-256 lookup from HF's metadata API), `download_model()` (streams to a `.part` file with an incrementally-updated hash, never buffering the whole file; verifies and only renames to the final name on success, deleting the partial file and naming both hashes on a mismatch). CLI: `Commands::LlamaCpp`/`LlamaCppCommands` (`List`/`Pull`/`Rm`) mirrors `Commands::Ollama` exactly, including the same "rebuild with `--features X`" error when disabled; `rm` prompts for confirmation (unlike `ollama rm`, since a self-downloaded GGUF is harder to reacquire). New optional dependency: `sha2` (gated behind `llama-cpp` only). 11/11 `llama_cpp_models` tests (two exercise a real download + checksum-mismatch-and-cleanup against a hand-rolled TCP mock server, mirroring `ollama_models.rs`'s own test pattern — no external mocking crate). No-feature build unaffected. |
| Phase 7 — TUI integration | ✅ Done (unverified against a real model/terminal) | Provider badge: `provider_icon()` (`src/tui/render.rs`) gained `"llama-cpp" => "⚙️"`, keyed to the exact string `LlamaCppProvider::name()` returns (confirmed by reading it, not assumed). Model Info panel (Ctrl+O): extended with GPU-layers/quantization rows via new `App::llama_cpp_model_details()`, shown only when the active provider is `llama-cpp`; context size stays generic (unchanged). New dialog per §7 point 4 option (a): `Ctrl+G` (`keys::is_llama_cpp_models`, confirmed free against the current `events.rs` key list, not the plan's stale snapshot), `AppMode::LlamaCppModelPicker`, new always-compiled `src/tui/llama_cpp_download.rs` (mirrors `ollama_download.rs`'s shape: local-file listing instead of a curated suggestions list, since there's no online search API to search *or* curate against; `spawn_download`/`spawn_delete`/`spawn_switch` background tasks; feature-gated real implementations with clear-error fallbacks when `--features llama-cpp` isn't compiled in, exactly like `ollama_download.rs`'s own not-compiled-in paths). Switching to a local model runs `LlamaCppProvider::new()` (which blocks while it loads the whole file) inside `tokio::task::spawn_blocking`, never on the async runtime; the built provider crosses back into `App` through a `PendingProvider` slot (`Arc<Mutex<Option<Arc<dyn Provider>>>>`) rather than riding on `TuiEvent`, since `Provider` trait objects aren't `Clone`/`Debug` (which `TuiEvent` derives) - the event only signals "check the slot". The dialog shows a genuine "Loading model…" state while this runs, per §4.5's blocking-swap requirement (deliberately not instant like Ollama's Ctrl+W). Status bar: no new mechanism needed - `llama_cpp_config`/`llama_cpp_download`/switch failures already flow through the existing generic `app.error_message` surface (`render_status_bar`) and through the same chat-notification pattern Ollama's dialogs already use for success/failure. **Corrected two stale plan assumptions found while implementing**: (1) `§7`'s file list named `components/dialogs/mod.rs` for the new dialog render function, but the Ollama dialogs actually live as free functions directly in `render.rs` (`render_model_download` et al.) - `render_llama_cpp_models` and its sub-views follow that real, current pattern instead. (2) Extracted the models-directory-resolution fallback (previously a private, `#[cfg(feature = "llama-cpp")]`-gated fn duplicated nowhere else) into `ProviderConfigs::llama_cpp_models_dir()` (`src/config/mod.rs`) so the CLI's `crustly llama-cpp` subcommand and the new TUI dialog share one implementation instead of the TUI reinventing it, exactly as the old function's own doc comment anticipated ("drop the cfg once a Phase 7 call site needs it unconditionally"). 24 new tests (9 `llama_cpp_download` unit tests covering both feature states, 9 `App` dialog-state-machine tests including the provider-swap-in-place path via the `PendingProvider` slot, 5 `render` snapshot tests, 1 new keybinding test) - 841/841 with `--features llama-cpp`, 815/815 with no features (all pre-existing Ollama dialog snapshot/behavior tests pass unmodified, confirming no regression per §7's own exit criterion). **Not verified**: no real `.gguf` file or live terminal session exists in this sandbox, so the actual "Loading model…" blocking-swap timing, live download progress rendering, and visual layout have not been driven end-to-end - only the state machine and static rendering are test-covered. |
...
| Phase 4b — Grammar-constrained tool calling | ✅ Done (mid-stream swap wired in, unverified against a real model) | `llguidance`/`toktrie` added as explicit optional deps; `llama-cpp-llguidance` feature; `src/llm/provider/llama_cpp_grammar.rs` (`tool_call_json_schema`/`build_tool_call_sampler`, unchanged from the infrastructure-only pass — see the git history for that landing). **The mid-stream swap is now wired into `run_complete`/`run_stream`** (`src/llm/provider/llama_cpp.rs`): `build_sampler()` gained an `Option<LlamaSampler>` grammar parameter, prepended first in the chain (before penalties/top-k/top-p/temp/dist - masked tokens simply never survive later ranking, matching llama.cpp's own `common_sampler` "grammar first" ordering); a per-model `ToolCallTokEnv` (`toktrie::TokEnv` behind the feature, `()` without it) is built once right after model load in `worker_loop` and threaded through `dispatch_job` alongside `chat_template`/`display_name`, never rebuilt per request. Both generation loops now track `generated_tokens: Vec<LlamaToken>` and, the moment accumulated text trims to a bare `{` (deliberately **not** the fenced-block case - a fence needs unconstrained prose before/after it, which a JSON-only grammar can't express, so that case is left to the always-on recovery heuristic exactly as before), call `try_build_constrained_sampler()` once: build the grammar sampler, `accept_many()` the tokens generated so far to sync its parser state, and swap it in as the sampler for the rest of that response. Runs decision: **the repo owner explicitly asked to proceed with this wiring** after the infrastructure-only pass's own writeup flagged it as unverified-without-a-real-model risk; re-reading `llguidance` 1.7.6's `Matcher::consume_tokens`/`with_inner` and `llama-cpp-2` 0.1.153's `llg_accept`/`llg_apply` directly (not assumed) while implementing found the actual failure mode is **not** a hang/panic/corruption as originally feared - a replay mismatch poisons the `Matcher` into an `Error` state, after which `llg_apply` is a documented no-op (`let Ok(mask) = ... else { return; }`) and `llg_accept` silently ignores further errors, so a bad replay just silently reverts to the exact unconstrained behavior Phase 4 already had, with the always-on recovery heuristic still catching whatever comes out. This materially changed the risk assessment from "could corrupt every request" to "worst case, no improvement for that one response" - safe enough to ship without live-model verification once understood, which the swap's own doc comment (`try_build_constrained_sampler`) and a new offline test (`try_build_constrained_sampler_does_not_panic_on_an_arbitrary_token_replay`, feeding it fabricated `LlamaToken`s the stand-in `TokEnv` never produced) both call out explicitly. 19/19 `llama_cpp` tests (2 new grammar-swap tests plus the not-compiled-in stub test), 4/4 `llama_cpp_grammar` tests - all still pass identically without the feature (847/847 with `--features llama-cpp-llguidance`, 842/842 with `--features llama-cpp`, 815/815 with none, 864/864 with `--features all-llm`, confirming this is genuinely additive/feature-gated, not a silent behavior change). **Still not verified end-to-end against a real `.gguf` model** - no such file exists in this sandbox, and huggingface.co is blocked by this environment's network policy (confirmed directly, not assumed) so one couldn't be fetched either; the manual verification §13's original exit criteria calls for (a real multi-tool agentic turn, confirming via a temporary log line that the recovery heuristic wasn't needed) remains outstanding for whoever next has a real model and terminal to run it against. **Post-wiring `/code-review` correction** (5 parallel review angles, 4 independently converging on the same core bug): the original swap trigger ("any bare `{`") was too loose - a legitimate JSON-shaped free-text answer (not a tool call at all) would get permanently hijacked into a fabricated, then-executed tool call whenever tools happened to be offered, which is nearly every turn. Fixed by replacing the trigger with the new `tool_call_recovery::commits_to_an_offered_tool_call` (requires the model to have already typed `"name": "<a real offered tool>"`, not merely opened a brace), shared by both loops instead of two divergent inline checks. `run_stream`'s trigger also used to check `pending_flush` (reset on every flush) while replaying `generated_tokens` (never reset) - the two windows disagreeing meant the swap either poisoned the matcher on any prose-then-call response (the exact case it targets) or fired on a resumed block of text rather than the response's true start; fixed by checking `full_text` (never reset) instead, matching what gets replayed. The swap also used to rebuild a fresh `penalties`/`dist` chain, silently discarding repeat-penalty history and re-seeding the RNG to literally replay the start of the pre-swap draw sequence; fixed by replaying `generated_tokens` into the *whole* assembled chain (`LlamaSampler::chain`'s `accept` propagates to every member, confirmed via `llama-sampler.cpp`'s `llama_sampler_chain_accept`) and perturbing the resolved seed by the token count already generated. Also fixed: `String::from_utf8_lossy` no longer runs unconditionally per token (only when a stop sequence or a still-live swap trigger needs it); `generated_tokens` stops accumulating once the swap is no longer possible or already decided; the `ParserFactory` (previously rebuilt per swap) is now cached alongside `TokEnv` once per model load as `ToolCallGrammarEnv`. 6 new tests (`commits_to_an_offered_tool_call`'s 4 cases in `tool_call_recovery.rs`, the seed-offset regression in `llama_cpp.rs`, factory reuse in `llama_cpp_grammar.rs`) - 853/853 with `--features llama-cpp-llguidance`, 847/847 with `--features llama-cpp`, 819/819 with none, 868/868 with `--features all-llm`. **Second `/code-review` pass, two agent-verified findings**: (1) `commits_to_an_offered_tool_call`'s `text.contains(&format!(...))` re-scanned the *entire* accumulated response on every token for as long as a response opened with `{` but never actually named an offered tool (confirmed: an O(n²) regression versus the pre-fix `O(1)` `starts_with('{')` check, and the common case per this row's own description - most responses aren't tool calls even when tools are offered) - fixed by bounding the scan to a fixed 256-byte leading window (`NAME_KEY_SCAN_WINDOW`) and dropping the `format!` allocations for a zero-alloc slice-based match, since a genuine top-level `"name"` key (the only shape `tool_instructions_block` ever instructs the model to produce) always appears within the first few dozen bytes of a real call. (2) The same substring search matched a `"name": "<tool>"` occurrence *anywhere* in the text, including nested inside an array/sub-object (e.g. `{"records": [{"name": "bash"}], ...}`) - confirmed real but "PLAUSIBLE" rather than an automatic hijack, since the `oneOf`/`additionalProperties:false` grammar would still poison-and-no-op on any response whose top-level shape didn't already happen to match a call; fixed anyway with a proper `{`/`}` depth tracker (string-escape-aware) requiring the match sit at depth 1, folded into the same windowed scan at no extra cost. A third finding (the swap trigger-and-apply block duplicated near-verbatim between `run_complete`/`run_stream`, the same class of drift risk that caused two of the first review's bugs) fixed by extracting both into one shared `maybe_swap_to_constrained_sampler`. 6 more new tests. 857/857 with `--features llama-cpp-llguidance`, 851/851 with `--features llama-cpp`, 823/823 with none, 872/872 with `--features all-llm`. |
...
session — code compiles, `cargo build`/`clippy -D warnings`/`test` are all
clean across every feature combination exercised, but end-to-end generation
quality/correctness — including tool-call recovery actually firing against
a real model's output, streaming's live token pacing, and the idle-unload/
reload cycle — is unverified). This is the single largest remaining
verification gap across every phase marked "Done" above; treat "Done" as
"implemented and offline-tested," not "verified against a real model."
