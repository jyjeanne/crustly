...
Status: **Phases 0–3 implemented** (streaming MVP with sampling/context
fixes; §0.1's Go/No-Go gate was explicitly overridden by the repo owner to
proceed — see below). Phase 4+ (tool calling, model management, TUI, GPU
...
| Phase 4+ | Not started | Tool calling (Phase 4/4b), performance-metrics TUI wiring (Phase 5 — the `PerfMetrics` plumbing exists and is now populated on both `complete()` and `stream()`, but hasn't been exercised against a real model or the TUI), model management (Phase 6), TUI integration (Phase 7), GPU acceleration (Phase 8), idle-unload hardening (Phase 9), docs/ADR (Phase 10). **No manual test against a real `.gguf` file has been run in any phase yet** (requires downloading a model, out of scope so far — code compiles and offline unit tests pass, but end-to-end generation quality/correctness, including streaming's live token pacing, is unverified). |
