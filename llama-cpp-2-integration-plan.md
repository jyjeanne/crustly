...
| Phase 7 — TUI integration | Not started | Model picker/download dialog (new keybinding + `AppMode` variant + `src/tui/llama_cpp_download.rs`, per §7 point 4's option (a) decision), Model Info panel extension, status-bar error surfaces. Provider badge and per-message perf footer need no dedicated work (already generic since Phase 5). Deferred: requires changes across `src/tui/events.rs`/`app.rs`/`components/dialogs/mod.rs`/`render.rs` that are hard to verify without visually driving the TUI, which this implementation pass couldn't do without a real terminal session and a downloaded model. |
