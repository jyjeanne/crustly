...
Status: **Phases 0–4, 6, 7, 8, 9, 10 implemented, Phase 4b infrastructure
implemented but not wired in** (streaming MVP with sampling/context fixes,
tool calling, model management, TUI integration, GPU backend features,
idle-unload, and docs/ADR; §0.1's Go/No-Go gate was explicitly overridden by
the repo owner to proceed — see below). Phase 4b's grammar/sampler
construction is built and tested (`src/llm/provider/llama_cpp_grammar.rs`)
but deliberately not called from the live decode loop yet — see that
phase's row below for why.
